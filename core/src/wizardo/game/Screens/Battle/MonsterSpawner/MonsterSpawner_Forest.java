...
public class MonsterSpawner_Forest extends MonsterSpawner {
    ...
    public void update(float delta) {
        ...
        if(!TEST_MONSTER_SPAWNED) {
            TEST_MONSTER_SPAWNED = true;
            Monster monster = new OrcMinion(screen, null, this);
        ...
        if(meleeSpawnTimer > 2f) {
            ...
            if(screen.monsterManager.liveMonsters.size() < maxMeleeMonsters) {
                for (int i = 0; i < spawnRatio; i++) {
                    Monster monster;
                    if(Math.random() >= 0.8f && stateTime > 0) {
                        monster = new OrcBrute(screen, null, this);
                    } else {
                        monster = new OrcMinion(screen, null, this);
                    }
                    spawnMonster(monster);
                }
            }
    ...
    public void spawnRangedMonsters() {
        if(rangedSpawnTimer > 3.2f && stateTime > 60) {
            rangedSpawnTimer = 0;
            if(screen.monsterManager.getRangedMonstersCount() < 30) {
                Monster monster = new OrcShaman (screen, null, this);
                spawnMonster(monster);
            }
        }
    }
        ...
        if(packTimer >= 15 && screen.monsterManager.liveMonsters.size() < maxMeleeMonsters) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if(Math.random() >= 0.85f) {
                    monster = new OrcBrute(screen, spawnPoint, this);
                } else {
                    monster = new OrcMinion(screen, spawnPoint, this);
                }
                spawnMonster(monster);
            }
        }
    ...
    public void spawnMonstersInEmptyQuadrant() {
        if(emptyQuadrantSpawnTimer > 2 && screen.monsterManager.liveMonsters.size() < maxMeleeMonsters) {
            ...
            for (int i = 0; i < 6 * spawnRatio; i++) {
                ...
                Monster monster = new OrcMinion(screen, position, this);
                spawnMonster(monster);
            }
        }
    }
